...
- Node/pnpm 安装：保持不变
- 构建命令：`pnpm run build`（Astro）
- 上传目录：`./dist`
- sitemap：在 workflow 中注入 `SITE` 环境变量（见 `.github/workflows/deploy.yml`），构建时生成 `dist/sitemap-index.xml`
...
- 通过 `astro-src/pages/404.astro` 生成 `dist/404.html`（GitHub Pages 需要），用于展示友好提示并引导回首页
